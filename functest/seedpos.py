...
def makepath(path, approachablePoints, visibilityParam, color, modelName):
...
#slicer.util.loadMarkupsFiducialList("E:/Program Files/Slicer 4.11.20210226/slicerscripts/addf1.fcsv")
startdata = np.loadtxt('E:/Program Files/Slicer 4.11.20210226/slicerscripts/NeedleStart3.dat',delimiter=',')
enddata  = np.loadtxt('E:/Program Files/Slicer 4.11.20210226/slicerscripts/NeedleEnds3.dat',delimiter=',')
...
print(woyaodeid) #4
np.reshape(startdata, (-1,3) )
np.reshape(enddata, (-1,3) )
nodecount = np.shape(startdata)[0]
PositionStart = []
PositionEnd = []
for i in range(nodecount):
    ps,pe = transfer(startdata[i],enddata[i])
    PositionStart.append(ps)
    PositionEnd.append(pe)
...
#modelReceived, allPaths, allPathPoints =makepath(allpath, nodecount,1, blue, "allPaths")
#iD = []
# itspos = []
# bvalues=[]
# avalues=[]
...
for j in range(len(woyaodeid)):#不用放主循环吧。。。。。
    ...
    iid = sp.findsphereid(tsl3,bs,woyaodeid[j])
    _,_,nbv = sp.localSubDivide(tsl3,iid) #后面循环没有生成nbv,导致只能算一次
    sti = str(indexlist[woyaodeid[j]])
    sti = sti.replace('\n','')
    ss = sti.split(',')
    #print('ss:'+str(ss))
    ppp1,ppp2,mer = minerrdir(points3,ss,startdata,woyaodeid[j],nbv,bspTreebone)#p1也和主循环无关，因为需要invcheck..还是从NS读
...
modelReceived, cPaths, cPathPoints =makepath(okpath, nodecount,1,blue , "readyPaths")
modelReceived, dPaths, dPathPoints =makepath(itspath, nodecount,1,red , "itsPaths")
modelReceived, dPaths, dPathPoints =makepath(citspath, nodecount,1,green , "citsPaths")
# f1 = open('E:/Program Files/Slicer 4.11.20210226/slicerscripts/Index2w.dat','w')
# f1.writelines(iD)
# f1.close()
#its =np.array(itspos)
#nbs =np.array(normb)
# np.savetxt('E:/Program Files/Slicer 4.11.20210226/slicerscripts/nintersectpoint2.dat', its, delimiter=',',fmt='%.8f') 
# np.savetxt('E:/Program Files/Slicer 4.11.20210226/slicerscripts/deln_ns2.dat', del_ps, delimiter=',',fmt='%.8f')  
# np.savetxt('E:/Program Files/Slicer 4.11.20210226/slicerscripts/deln_ne2.dat', del_pe, delimiter=',',fmt='%.8f')
# np.savetxt('E:/Program Files/Slicer 4.11.20210226/slicerscripts/deln_nos2.dat', del_pos, delimiter=',',fmt='%.8f')  #origin，没转换过的
# np.savetxt('E:/Program Files/Slicer 4.11.20210226/slicerscripts/deln_noe2.dat', del_poe, delimiter=',',fmt='%.8f')
# np.savetxt('E:/Program Files/Slicer 4.11.20210226/slicerscripts/livernb2.dat', nbs, delimiter=',',fmt='%.8f')
# np.savetxt('E:/Program Files/Slicer 4.11.20210226/slicerscripts/del_b2.dat', bvalues, delimiter=',',fmt='%.8f')
# np.savetxt('E:/Program Files/Slicer 4.11.20210226/slicerscripts/del_a2.dat', avalues, delimiter=',',fmt='%.8f')
...
# 8
# >>> slicer.util.loadMarkupsFiducialList("E:/Program Files/Slicer 4.11.20210226/slicerscripts/addf2.fcsv")
